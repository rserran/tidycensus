...
For the ACS, use either `"acs1"` or `"acs5"` for the ACS detailed tables, and append `/profile` for the Data Profile, `/subject` for the Subject Tables, or `/cprofile` for the Comparison Profile.  To browse these variables, assign the result of this function to a variable and use the `View` function in RStudio.  An optional argument `cache = TRUE` will cache the dataset on your computer for future use.
