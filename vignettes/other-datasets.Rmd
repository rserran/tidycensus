...
By default, specifying `"population"`, `"components"`, or `"housing"` (housing is not available for 2020 and later) as the `product` in `get_estimates()` returns all variables associated with that component.   For example, we can request all components of change variables for US states in 2024: 
